...
7. **Feature selection** (optional, controlled by `FS_METHOD`):
   ...
   - **Slot-aligned voting CV aggregation** (`cv_aggregate_select_aligned`): each fold fits its own per-band CSP and independently selects its own subset, same as before — but each fold's CSP is a *different fit* (different training subset) than the **final** CSP fit on 100% of the data. `component_order='alternate'` only guarantees a consistent *role* ordering across fits (filter-for-class-A, filter-for-class-B, …), not that a given slot's literal spatial filter is identical across fits — at low SNR this can drift. Before counting votes, each fold's slots are re-aligned onto the final CSP's slot numbering via `match_csp_filters` (Hungarian assignment on sign-invariant cosine similarity between filter rows); a vote is dropped if the matched similarity is below `sim_floor` (0.5) and otherwise weighted by it. Slots whose weighted vote total is `≥ ⌈K_FOLDS/2⌉` form the *stable set*. Falls back to a wide relevance-ranked pool if too few slots clear that threshold
   - **Redundancy trim** (`_mrmr_trim`, mRMR only): the stable/fallback pool is pruned to `FS_K_TOP_MAX` by greedy relevance-minus-redundancy, computed on the **final** CSP's feature values — so the deployed set is non-redundant, not just stable. This applies to the fallback path too, which previously had no redundancy check
   - The notebook prints per-fold `"FS kept N feat: [...]"` and a final table with rank/idx/band/component/**weighted votes**/**mean filter sim**/mean-relevance — `mean filter sim` close to 1 means that slot's spatial filter is stable between the fold fits and the final all-data fit (the index transfer is trustworthy); close to 0 means the slot's physical content drifted and the kept feature should be treated with more skepticism
...
### §5b. Known limitations of the feature-selection pipeline

The slot-aligned voting + redundancy trim (step 7) removes the two main correctness gaps in the feature-selection process (silent index/filter mismatch between fold-CSPs and the final CSP; no redundancy check in the fallback path). Reviewed against publication-grade rigor, point by point:

- **Grid search and CV reuse the same fold split — confirmed, not a false alarm**: both `sgkf = StratifiedGroupKFold(n_splits=K_FOLDS)` calls (Cell 4 and Cell 5) use `shuffle=False` (the sklearn default, never overridden), so given the same `X_epochs`/`y_epochs`/`trials` they are **deterministic and produce the identical split** every time. `N_CSP_COMPONENTS`/`FS_K_TOP_MAX` are therefore chosen to maximise accuracy on the exact same folds that later produce the reported `mean_acc`/`std_acc` — a real double-dip (~5% optimistic bias per internal testing), not avoided by the grouping/stratification itself (those control *what goes into* each fold, not *whether the two cells see the same folds*). Decision pending: leave as-is (acceptable for per-subject deployment) or decouple by giving Cell 4 and Cell 5 different `random_state` with `shuffle=True` — flag to revisit before any cross-subject/method comparison claim.
- **`FS_METHOD='mrmr'`**: kept as-is by design decision — assumed the right choice on theoretical grounds (explicit redundancy control matters here because `BANDS` are deliberately overlapping), not empirically re-validated against `'fisher'`/`'mibif'` on this data. Not pursued further by request.
- **`sim_floor=0.5` / majority threshold sensitivity, and a permutation test against chance**: both considered and intentionally not pursued by request — not part of the current scope.
- **Single subject per trained model**: by design, not a gap — `create_slda.ipynb` produces one personalised CSP+sLDA per subject (a calibration session is inherently per-subject). Cross-subject generalisation is a question for the *evaluation*-time pipeline (`matlab_simulation`'s `main_group_analysis.m`, which already pools real-session and counterfactual metrics across subjects), assessed once a given subject's model is in active use — not a property the training notebook itself needs to demonstrate.
- **Fixed — Cell 6 calibration diagnostic plot now reflects the deployed Platt scaling**: `prob_tr`/`prob_cv` feeding figure (e) ("Calibration: train vs CV test … Platt scaling applied to train") previously computed the *raw* sigmoid of the LDA decision score, not `σ(platt_a · score + platt_b)` — the title claimed Platt scaling while the plotted curves didn't use it. Both now apply the actual fitted `platt_a`/`platt_b` (matching what `slda.py` does at inference), so the figure honestly shows the calibration that ships. `prob_class1_tr` (used by the separate confidence-over-time figure) intentionally still uses the raw sigmoid, to stay shape-comparable with Cell 5's pre-Platt CV Fig 2.
